...
> If you are a seasoned web developper, all you need to know is that this project consists of a few react packages to help you build your presentation, which you write using [MDX](https://mdxjs.com/).  
...
<div style={{width: '100%', textAlign: 'center'}}>
  <img alt="Pipeline" src="public/pipeline.svg" width="75%"/>
</div>
...
An example where we transform the markdown syntax is with images.
In order for files to be processed by Webpack, you need to import them and images are no different.
Because this is not handy when writing markdown, we created a Unified plugin that transforms the image syntax:
```jsx
![ALT_TEXT](./path/to/image.png)

// This gets transformed automatically to:
import imported_image from './path/to/image.png';
